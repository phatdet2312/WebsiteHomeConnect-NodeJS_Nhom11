const repo = require('../../repositories/admin/hienTrangRepository');

class HienTrangService {
    ...
    async update(id, dto) {
        ...
        if (!item) throw new Error('Không tìm thấy hiện trạng');
        const err = dto.validate();
        if (err) throw new Error(err);
...
module.exports = new HienTrangService();